...
- 仅当用户粘贴长度不少于 64 的 Base64/Base64URL 候选 Token 时，Skill 才会通过 `GET /user/info` 验证；验证成功后保存到当前用户的 `${CODEX_HOME}/cpris-wxapp-rest-api/credentials.json`（未设置时为 `~/.codex/cpris-wxapp-rest-api/credentials.json`）。每次后续调用都会从该路径读取认证信息。
