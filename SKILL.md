...
5. 根据 [请求与响应约定](references/schemas.md) 检查 HTTP 状态和业务 code。仅交付成功响应中的数据；儿童姓名按返回原文直接显示，不做额外脱敏，不猜测被遮盖内容。
...
- 儿童姓名（含昵称、曾用名及对应拼音）按成功响应原文直接显示，不做额外脱敏；其他敏感字段保持脱敏形态。若后端已遮盖姓名，不猜测或还原。当前后端部分错误和非 JSON 响应直接透传，脚本不输出这类原文；不能宣称所有响应均保证脱敏。
