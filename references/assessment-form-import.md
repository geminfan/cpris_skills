...
1. 用 `/childrenInfo/page` 查找儿童。优先以姓名加出生日期等第二标识匹配；只有姓名且有重名、信息冲突或附件儿童与用户指定儿童不一致时，停止写入并请用户选定。若账号没有 ai_show_child_name 权限或返回姓名已遮盖，不能按掩码猜测对应儿童；结合用户明确提供的 childId、出生日期等可用标识确认，无法唯一匹配时请用户指定儿童后再写入。不能因附件中出现儿童资料就自动创建档案。
